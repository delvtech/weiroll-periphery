...
contract Router is VM, ReentrancyGuard {
    ...
    /// @param amounts        Amount of ERC-20 tokens 
    /// @return data          Data that would get return by the commands execution.
    function execute(bytes32[] calldata commands, bytes[] calldata state, IERC20[] calldata approvalTokens, uint256[] calldata amounts) external payable nonReentrant returns (bytes[] memory data) {
        uint256 noOfApprovalTokens = approvalTokens.length;
        // Validate: Length of approvalTokens array should equal to amounts array length.
        Assert.check(amounts.length == noOfApprovalTokens, Error.Type.IncorrectArrayLength);

        uint256[] memory selfBeforeBalances = new uint256[](amounts.length);
        ...
        for (uint256 i = 0; i < noOfApprovalTokens; i++) {
            ...
            selfBeforeBalances[i] = approvalTokens[i].balanceOf(address(this));
            // Move funds to the contract.
            approvalTokens[i].safeTransferFrom(msg.sender, address(this), amounts[i]);
        }
        data = _execute(commands, state);

        // Make sure at the end of the execution the balance delta is zero.
        for (uint256 i = 0; i < noOfApprovalTokens; i++) {
            Assert.check(selfBeforeBalances[i] == approvalTokens[i].balanceOf(address(this)), Error.Type.UnAccountedERC20Balance);
        }
